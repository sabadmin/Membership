    ...
    if request.method == 'POST':
        ...
        with get_tenant_db_session(tenant_id) as s:
            ...
            try:
                ...
                session['user_name'] = new_user.email # Initial user_name is email
                session['tenant_name'] = Config.TENANT_DISPLAY_NAMES.get(new_user.tenant_id, new_user.tenant_id.capitalize()) # Set tenant_name
...
def login():
    ...
    if request.method == 'POST':
        ...
        with get_tenant_db_session(tenant_id) as s:
            ...
            # UPDATED: If user not found, redirect to register
            if not user:
            ...
            # If user found, proceed with password and active status check
            ...
            if user.check_password(password):
                ...
                session['tenant_name'] = Config.TENANT_DISPLAY_NAMES.get(user.tenant_id, user.tenant_id.capitalize()) # Set tenant_name
    ...
    if request.method == 'POST':
        ...
        try:
            with get_tenant_db_session(g.tenant_id) as session:
                ...
                session.flush() # Get new_user.id before creating UserAuthDetails
                new_auth_details = UserAuthDetails(
                    user_id=new_user.id,
                    tenant_id=new_user.tenant_id,
                    is_active=True,
